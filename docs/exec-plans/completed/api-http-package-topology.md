...
| Task | Status | Notes |
| --- | --- | --- |
| API-HTTP-001 | completed | Relocated `packages/http-api` to `packages/api/http`, renamed the package to `@taxkit/api-http`, updated runtime imports/config and passed mandatory gates. |
| API-HTTP-002 | completed | Refreshed current docs and public references to `packages/api/http` and `@taxkit/api-http`; no new Changeset required beyond the existing rename Changeset. |
...
- Renamed package metadata and current runtime/source/config imports from
  `@taxkit/http-api` to `@taxkit/api-http`.
...
- `bun run --filter=@taxkit/api-http test` first exposed missing local package
...
- Source/config import audit over current app/package runtime surfaces found no
  remaining `@taxkit/http-api` or `packages/http-api` references.
...
- `bun run changeset status --verbose` passed and reports patch impact for
  `@taxkit/api-http` plus the repo fixed group.
- Parent review hardened the SDK boundary check to reject both
  `@taxkit/api-http` and stale `@taxkit/http-api` imports or package
...
- Refreshed current docs and public references so root maps, architecture docs,
  ...
  HTTP API package as `packages/api/http` / `@taxkit/api-http`.
- Updated `docs/product-specs/api-http-package-topology.md` so old
  `packages/http-api` / `@taxkit/http-api` references describe the original
...
- Current-docs old-name audit over root docs, app/package READMEs, public MDX,
  ...
  found no `@taxkit/http-api` or `packages/http-api` matches.
- Current-docs target-name audit confirms those same surfaces use
  `@taxkit/api-http` and `packages/api/http` for the implemented package.
...
- `bun run changeset status --verbose` passed and reports the existing
  ...
  `@taxkit/api-http` plus the fixed release-train packages.
...
- Verified `packages/api/http/package.json` names `@taxkit/api-http`, keeps
...
- Verified root workspace metadata includes `packages/api/*`, `bun.lock`
  resolves `@taxkit/api-http` to `workspace:packages/api/http`, Knip points
...
- Verified current app/package source imports:
  `apps/api/src/server.ts` imports `@taxkit/api-http/server`; `apps/web`
  source imports only `@taxkit/api-http/client`,
  `@taxkit/api-http/client/live` and `@taxkit/api-http/config`; SDK source
  and package metadata have no `@taxkit/api-http` or `@taxkit/http-api`
  dependency.
- Verified the SDK boundary guard still rejects both `@taxkit/api-http` and
  stale `@taxkit/http-api` SDK imports while requiring the HTTP API package
  to depend on `@taxkit/sdk`.
- `bun run --filter=@taxkit/api-http test` passed with 1 file and 2 tests.
- `bun run --filter=@taxkit/api-http check-types` passed.
- `bun run --filter=@taxkit/api-http build` passed.
- `bun run --filter=api check-types` passed.
- `bun run --filter=api build` passed.
- `bun run --filter=web check-types` passed.
- `bun run --filter=@taxkit/sdk check-boundaries` passed with
...
- `bun run changeset status --verbose` passed. It reports
  ...
  `@taxkit/api-http` to `0.0.3`; because this repo uses a fixed release
  group, the status also reports patch bumps for `@taxkit/core`,
  `@taxkit/rules-au-income-tax`, `@taxkit/rules-au-pay`,
  `@taxkit/rules-au-stsl`, `@taxkit/sdk`, `@taxkit/testing`,
  `@taxkit/tsconfig` and `@taxkit/calculators`. No minor or major release
  impact is reported.
...
- API smoke: `GET /api/docs/openapi.json` returned `200`,
  `info.title = "TaxKit API"`, 10 paths and confirmed entries for
...
- Parent acceptance reran `bun run --filter=@taxkit/api-http test`,
  `bun run --filter=@taxkit/sdk check-boundaries`, `bun run docs:validate`,
...
- Audit pass 1: production and test call graphs still match the target spec:
  `apps/api` imports `@taxkit/api-http/server`, `apps/web` imports
  browser-safe `@taxkit/api-http/client`, `@taxkit/api-http/client/live` and
  `@taxkit/api-http/config`, and HTTP API tests still use the in-process
  client over `TaxKitServerLayer`.
- Audit pass 2: package boundaries remain intact. `apps/api` still owns Bun
  server startup and process lifecycle, `@taxkit/api-http` owns HTTP transport
  contracts/layers, `@taxkit/calculators` owns calculator orchestration and
  `@taxkit/sdk` has no `@taxkit/api-http` dependency.
...
- Audit pass 2: public docs stay focused on the open-source TaxKit API, SDK,
  calculator and docs surfaces. No private reference-repo details were added.
- Audit pass 3: historical `@taxkit/http-api` references remain only in
  ...
  guidance uses `@taxkit/api-http`.
- Call-graph audit: the docs still match the spec's target graph:
  `apps/api` serves `@taxkit/api-http/server`, public API docs describe the
  route contract as `@taxkit/api-http`, and `apps/web` guidance stays on
  browser-safe `@taxkit/api-http/client` exports.
...
- Audit pass 1: full source/import graph matches the target production and
  ...
  `@taxkit/api-http/server`; `ApiRoutesLive` owns API routes, generated docs
  and OpenAPI JSON; `CalculatorApiHandlerLive` calls
  `@taxkit/sdk/effect` `calculateRunRequest`; HTTP API tests use
  `@taxkit/api-http/client/server` over `TaxKitServerLayer`; `apps/web`
...
- Audit pass 2: package metadata, workspaces, lockfile, filters, Changeset and
  ...
  `packages/api/http` / `@taxkit/api-http`; `packages/http-api` is absent;
  `.changeset/api-http-package-topology.md` targets `@taxkit/api-http` only;
  fixed release-train patch bumps are reported by Changesets but no
  `@taxkit/http-api` package bump exists.
...
- Product specs and task lists with `status: implemented` still contain
  historical `@taxkit/http-api` and `packages/http-api` wording from their
  ...
  guidance.
- Changelog and Changeset history still mention `@taxkit/http-api` to explain
