...
`packages/http-api` and was named `@taxkit/http-api`. Existing architecture
docs already identified the long-term shape as `packages/api/http` with
package name `@taxkit/api-http`. This spec moved that planned placeholder
...
`packages/api/http` and is named `@taxkit/api-http`; `apps/api` remains the
...
- original implementation: `packages/http-api` / `@taxkit/http-api`
- planned architecture: `packages/api/http` / `@taxkit/api-http`

That mismatch was still cheap to fix because `@taxkit/http-api` is private and
...
HTTP API tests
  -> @taxkit/api-http/client/server TaxKitApiInProcessClientLive
    -> @taxkit/api-http/server TaxKitServerLayer
      -> CalculatorApiHandlerLive
        -> @taxkit/sdk/effect calculateRunRequest
...
Package boundary audits
  ...
  -> no dependency on @taxkit/api-http
  -> browser code imports only @taxkit/api-http/client or browser-safe subpaths
```

## Goals

- Move the implemented HTTP API package to `packages/api/http`.
- Rename the package to `@taxkit/api-http`.
...
- Keep `apps/web` as an HTTP client of `apps/api`, not an in-process API
  mount.
- Keep `@taxkit/sdk` independent from the HTTP API package.
...
- Leave historical specs, completed execution plans and changelogs readable;
  when they mention `@taxkit/http-api`, the historical context should be
  explicit.
...
- Do not introduce compatibility aliases or pass-through packages for
  `@taxkit/http-api` unless implementation discovers a concrete private
...
`@taxkit/calculators` owns reusable calculator orchestration, metadata
projection, graph assembly, calculation dispatch and expected error shaping.

`@taxkit/sdk` owns public SDK facades and remains independent of
`@taxkit/api-http`. HTTP transport may import the SDK; the SDK must not import
HTTP transport.

Browser and app runtime code must consume browser-safe package exports. Server
exports such as `@taxkit/api-http/server`, handler layers and in-process
...
4. Preserve the existing export map shape with the new package name:
   `@taxkit/api-http`, `@taxkit/api-http/api`,
   `@taxkit/api-http/client`, `@taxkit/api-http/client/live`,
   `@taxkit/api-http/client/server`, `@taxkit/api-http/config`,
   `@taxkit/api-http/server`, `@taxkit/api-http/handlers` and
...
- `@taxkit/api-http`: patch Changeset describing the package rename from
  `@taxkit/http-api` and move from `packages/http-api` to
  `packages/api/http`.

No `@taxkit/http-api` Changeset is expected after the rename because that is
...
- Runtime source imports use `@taxkit/api-http` and target call graphs still
  match the implementation.
- `apps/api` still owns process startup and serves the HTTP API package route
  layer.
- `apps/web` imports only browser-safe `@taxkit/api-http` client exports.
- `@taxkit/sdk` package metadata and source still have no dependency on
  `@taxkit/api-http`.
- Current docs, READMEs, public MDX docs, root maps and architecture docs use
  the target package path/name.
- Historical docs that keep `@taxkit/http-api` do so only as historical
  context.
