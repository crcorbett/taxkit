...
| Production dependency graph | Change | `knip.production.json`, root `package.json`, `knip.json` only if shared development entry definitions change, the eight code-bearing artifact package manifests/entrypoints when a finding requires correction, `packages/scripts/package.json` and executable runtime entries, and `apps/api/package.json` plus `src/index.ts`. `packages/tsconfig` and root `tools/**` are N/A for this production source graph for the reasons above. |
...
| Agent instructions and skills | N/A | Root `AGENTS.md`, `.agents/skills/prd-writer`, `.agents/skills/prd-implementer` and `.agents/skills/docs-writer` already encode ownership, task protocol and documentation routing; this rollout does not change that workflow. |
