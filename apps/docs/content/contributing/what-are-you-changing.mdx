...
description: "Route a contribution idea to the right TaxKit guide and evidence."
status: "draft"
---
...
| ----------------------------------------------------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| Add an input fact, derived fact or question prompt                                  | [Add a fact](./add-a-fact.mdx)                           | Usually `@taxkit/core` for shared descriptors, or the owning rule package for rule-specific facts |
| Add or change a tax formula, threshold, parameter table or source-backed derivation | [Add a rule](./add-a-rule.mdx)                           | `@taxkit/rules-au-pay`, `@taxkit/rules-au-income-tax` or `@taxkit/rules-au-stsl`                  |
| Add a public calculation goal, request context or report shape                      | [Add a calculator](./add-a-calculator.mdx)               | Rule package for the calculator program, `@taxkit/calculators` for reusable orchestration         |
...
| Change HTTP endpoint shape or SDK helper shape                                      | [Backward compatibility](./backward-compatibility.mdx)   | `@taxkit/api-http` for transport contracts, `@taxkit/sdk` for SDK facades                         |
